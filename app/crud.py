...
def _build_addresses(payloads: list[AddressCreate]) -> list[Address]:
    """Fresh Address rows in the order the client sent them."""
    return [Address(position=index, **item.model_dump()) for index, item in enumerate(payloads)]
...
def replace_contact(db: Session, contact: Contact, payload: ContactReplace) -> Contact:
    ...
    # Full replacement: delete-orphan cascade removes the rows that fall out of the list.
